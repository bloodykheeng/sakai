...
function ListPage({ loggedInUserData, ...props }) {
    ...
    const { data, isLoading, isError, error, status } = useQuery(["service-types"], getAllServiceTypes, {
        onSuccess: (data) => {
            console.log("list of all getAllServiceTypes : ", data);
        },
        onError: (error) => {
            console.log("Error fetching getAllServiceTypes is : ", error);
    ...
    const DeleteSelectedItemMutation = useMutation((variables) => deleteServiceTypeById(variables), {
        onSuccess: (data) => {
    ...
    const columns = [
...

        {
            ...
            field: "fee",
            render: (rowData) => {
                const amount = parseFloat(rowData.fee.replace(/,/g, ""));
                return <div>{isNaN(amount) ? rowData.fee : amount.toLocaleString(undefined, { maximumFractionDigits: 2 })}</div>;
            },
        },
        {
            title: "Description",
    ...
    return (
        <div style={{ width: "100%" }}>
            ...
            <Panel header="Service Types" style={{ marginBottom: "20px" }}>
                <div style={{ height: "3rem", margin: "1rem", display: "flex", justifyContent: "flex-end", gap: "1rem" }}>
                    {activeUser?.permissions.includes("create") && <Button label="Add Service Type" className="p-button-primary" onClick={() => setShowAddForm(true)} />}
