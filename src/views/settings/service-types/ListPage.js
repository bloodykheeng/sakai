...
function ListPage({ loggedInUserData, ...props }) {
    ...
    const { data, isLoading, isError, error, status } = useQuery({ queryKey: ["service-types"], queryFn: getAllServiceTypes });
    console.log(data);
    useEffect(() => {
        if (isError) {
    ...
    const DeleteSelectedItemMutation = useMutation({
        mutationFn: (variables) => deleteServiceTypeById(variables),
        onSuccess: (data) => {
